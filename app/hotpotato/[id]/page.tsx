...
const HotPotatoPage = () => {
  ...
  return (
    <div className={theme.canvas}>
      ...
      <div className={`${theme.card.wrapper} w-full max-w-md bg-slate-900/60 border-slate-800/80 shadow-2xl relative z-10`}>
        ...
        {/* بخش تایمر دایره‌ای با تم سایبرپانک */}
      ...
      {players && players.length > 0 && (
        <div className="w-full max-w-md mt-8 relative z-10">
          <div className="flex items-center justify-between mb-3 px-1">
            <span className="text-[11px] font-bold text-slate-500 uppercase tracking-wider">بازیکنان حاضر</span>
            <span className="text-[10px] font-bold bg-slate-900 text-slate-400 px-2 py-0.5 rounded-full border border-slate-800">{players.length}</span>
          </div>
          <div className="flex flex-wrap gap-2 p-3 bg-slate-900/20 border border-slate-900/60 rounded-xl backdrop-blur-sm">
            {players.map((p, index) => (
              ...
              >
                <span className={`w-1.5 h-1.5 rounded-full ${p === player ? 'bg-cyan-400 animate-pulse' : 'bg-slate-600'}`} />
                {p}
              </div>
            ))}
          </div>
